<template>
    <div class="assets-doc-page">
        ...
        <div class="card-grid" v-loading="loading">
            <div
                ...
                <div class="doc-container">
                    <div v-if="item.coverUrl" class="doc-preview">
                        <!-- 文档封面图片 -->
                        <img :src="item.coverUrl" alt="文档封面" class="doc-cover" />
                        <!-- 文件格式标签 -->
                    ...
                    <div v-else class="doc-placeholder">
                        <FileDoc theme="outline" size="48" fill="#909399" />
                        <span>暂无文档</span>
                    </div>
                </div>
        ...
        <!-- 空状态 -->
        <el-empty v-if="!loading && docList.length === 0" description="暂无数据" />
        ...
        <!-- Drawer组件 -->
        <PDrawer
            :title="getDetailOperation(detailFormModel).op === 'create' ? '添加文档' : '编辑文档'"
...
import { Plus, Edit, Delete, Down, Copy, Forbid, Up, Upload, Download, Star, FileDoc, FileText, Eyes } from '@icon-park/vue-next';
import { assetsDoc, assetsClassification, assetsCollection } from '@/api/index.ts';
import provideForm from '@/common/provideForm';
import { formatFileSize } from '@/common/utils';
import moment from 'moment';

// 配置provideForm
const { searchFormModel, isTableDataLoading, queryTableData, formColumns, formRules: drawerFormRules, isOpenDetail, detailFormModel, getDetailOperation, watchDetailForm } = provideForm([
    {
        label: 'id',
        prop: 'id',
        isDetail: true,
    ...
    },
    {
        ...
        isUpdate: true,
        domType: 'uploadFile',
        uploadPath: 'assets/doc',
        rules: [
            { required: true, message: '文档文件必填', trigger: 'blur' }
        ],
        componentProps: {
    ...
    },
    {
         ...
         isUpdate: true,
         domType: 'uploadImage',
         uploadPath: 'assets/doc/cover',
         rules: [
             { required: true, message: '文档封面必填', trigger: 'blur' }
         ]
     },
    {
        label: '描述',
    ...
    },
    {
        ...
        domType: 'select',
        domData: async (key) => {
            const result = await assetsClassification.query({ parentId: 3, name: key });
...
// 加载文档分类数据
const loadDocClassifications = async () => {
    const classifications = await assetsClassification.query({ parentId: 3 });
    docClassifications.value = convertToTreeFormat(classifications.records || []);
};
...
const handleSetHandpick = async (item, handpick) => {
    try {
        ...
        if (handpick === true) {
            await assetsDoc.setHandpick(item.id);
        } else {
            await assetsDoc.cancelHandpick(item.id);
        }
...
const handleOffShelf = async (item) => {
    try {
        ...
        await assetsDoc.offShelf(item.id);
...
const handleOnShelf = async (item) => {
    try {
        ...
        await assetsDoc.onShelf(item.id);
...
const handleDisable = async (item) => {
    try {
        await ElMessageBox.confirm('确定要禁用此文档吗？', '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
        });
        
        await assetsDoc.update({ id: item.id, status: 0 });
...
const handleEnable = async (item) => {
    try {
        await assetsDoc.update({ id: item.id, status: 1 });
...
const handleDelete = async (item) => {
    try {
        await ElMessageBox.confirm('确定要删除此文档吗？删除后无法恢复。', '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
        });
        
        await assetsDoc.del(item.id);
