<template>
    <div class="assets-image-page">
        ...
        <!-- Drawer组件 -->
        <PDrawer
            :title="getDetailOperation(detailFormModel).op === 'create' ? '添加图片' : '编辑图片'"
...
import { Plus, Edit, Delete, Down, Copy, Forbid, Up, Upload, Download, Star } from '@icon-park/vue-next';
import { assetsImage, assetsClassification, assetsCollection } from '@/api/index.ts';
...
const { searchFormModel, isOpenDetail, detailFormModel, getDetailOperation, watchDetailForm } = provideForm([
    ...
    },
    {
        ...
        isUpdate: true,
        rules: [
            { required: true, message: '图片标题必填', trigger: 'blur' },
    ...
    // TODO：el-tree-select无法回填数据
    {
        ...
        domType: 'select',
        domData: async (key) => {
            const result = await assetsClassification.query({ parentId: 1, name: key });
            // return convertToTreeFormat(result.records || []);
    ...
    },
    {
        ...
        domData: async (key) => {
            ...
            const format = result.records.map((item) => ({
                label: item.name,
                value: item.id
            }));
...
const imageClassifications = ref([]);
...
// 加载图片分类数据
const loadImageClassifications = async () => {
    const classifications = await assetsClassification.query({ parentId: 1 });
    imageClassifications.value = convertToTreeFormat(classifications.records || []);
};
...
const queryData = async (params: any = {}) => {
    ...
    try {
        ...
        // 处理时间范围参数
        if (queryParams.createdAt && queryParams.createdAt.length === 2) {
        ...
        const result = await assetsImage.query(queryParams);
...
const handleEdit = (item) => {
    const detail = {
        ...
        fileSizeFormat: formatFileSize(item.fileSize),
    }
...
const handleCreate = async (formData: any) => {
    await assetsImage.create(formData);
    ElMessage.success('创建成功');
};

// 更新处理函数
const handleUpdate = async (formData: any) => {
    await assetsImage.update(formData);
    ElMessage.success('更新成功');
};

// 格式化数据处理函数
const handleFormatData = ({ queryData, saveData }: any) => {
    // 可以在这里处理数据格式化逻辑
    if (saveData) {
        // 保存时的数据处理 - 将文件大小转换为字节(B)
        if (saveData.fileSize) {
            // 如果fileSize是字符串格式(如"1.5MB", "500KB")，转换为字节数
            if (typeof saveData.fileSize === 'string') {
                const sizeStr = saveData.fileSize.toUpperCase();
                let sizeInBytes = 0;
                
                if (sizeStr.includes('MB')) {
                    sizeInBytes = parseFloat(sizeStr.replace('MB', '')) * 1024 * 1024;
                } else if (sizeStr.includes('KB')) {
                    sizeInBytes = parseFloat(sizeStr.replace('KB', '')) * 1024;
                } else if (sizeStr.includes('B')) {
                    sizeInBytes = parseFloat(sizeStr.replace('B', ''));
                } else {
                    // 如果没有单位，假设是字节
                    sizeInBytes = parseFloat(sizeStr);
                }
                
                saveData.fileSize = Math.round(sizeInBytes);
            }
            // 如果已经是数字，确保是整数字节
            else if (typeof saveData.fileSize === 'number') {
                saveData.fileSize = Math.round(saveData.fileSize);
            }
        }
    }
    if (queryData) {
...
const handleOffShelf = async (item) => {
    try {
        ...
        await assetsImage.offShelf(item.id);
...
const handleOnShelf = async (item) => {
    try {
        ...
        await assetsImage.onShelf(item.id);
...
const isTitleOverflow = (title) => {
    ...
    // 获取卡片容器的实际宽度，减去padding和按钮区域的宽度
    // 卡片宽度约为280px，减去左右padding(32px)和按钮区域(约80px)，剩余约168px
    const maxTitleWidth = 168;
    return textWidth > maxTitleWidth;
};
