        ...
        <el-col
            ...
            :lg="
                item.domType === 'datePickerRange'
                    ? 8
                    : item.domType === 'dateTimePickerRange'
                    ? 10
                    : 6
            "
            :xl="
            ...
            <FormItem
                :formColumnItem="formatFormColumn(item)"
                :formModel="props.formModel"
                :onWhere="props.onWhere" />
            <slot :name="`${onWhere}-${item.prop}-after`" />
            <slot :name="`${onWhere}-${index}-after`" />

            <slot
                v-if="index === props.formColumns.length - 1"
                :name="`${onWhere}-last`" />
        </el-col>
    </el-row>
 ...
 * @property {'search' | 'detail'} onWhere - 表单所在位置标识,可选值:'search'(搜索表单)/'detail'(详情表单)
 */
...
const formatFormColumn = (item: FormColumnsInterface): FormColumnsInterface => {
    ...
    formattedItem.placeholder =
        item[placeholder] ||
        item.placeholder ||
        (['select', 'radio'].includes(item.domType) &&
        props.onWhere === 'search'
            ? '全部'
            : '');
