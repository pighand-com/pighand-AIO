...
public class CheckInRecordService extends BaseServiceImpl<CheckInRecordMapper, CheckInRecordDomain> {
    ...
    public CheckInRecordDomain findTodayRecord(Long userId, Long locationId, LocalDate checkInDate) {
        return super.queryChain()
            .where(CHECK_IN_RECORD.USER_ID.eq(userId))
            .and(CHECK_IN_RECORD.LOCALTION_ID.eq(locationId))
            .and(CHECK_IN_RECORD.CHECK_IN_AT.eq(checkInDate))
            .one();
    }
     ...
     * @return
     */
    public List<CheckInRecordDomain> findTodayRecords(Long userId, LocalDate checkInDate) {
        return super.queryChain()
            .where(CHECK_IN_RECORD.USER_ID.eq(userId))
            .and(CHECK_IN_RECORD.CHECK_IN_AT.eq(checkInDate))
            .list();
